...
// By  Adedokun Adewale Azeez 14th August 2019
        ...
        $sql_statement = "SELECT * FROM block_assign WHERE username='$username' AND password='$password'";
        $result = mysqli_query($connect, $sql_statement);

        if (mysqli_num_rows($result)) {
            while ($rows = mysqli_fetch_assoc($result)) {
                $_SESSION['id_bl'] = $rows['id'];
                $_SESSION['name_id_bl'] = $rows['username'];
                $_SESSION['password_bl'] = $rows['password'];
                $_SESSION['bl_full_name'] = $rows['name'];
                $_SESSION['imagess'] = $rows['images'];
                $_SESSION['svn'] = $rows['svn'];
                $_SESSION['block'] = $rows['block'];
                $_SESSION['phone'] = $rows['phone'];
                //  echo "<div style='color:white; margin-top:6px;'>Your Login is successfull</div>";
            }
            $_SESSION['welcome'] = "Welcome";
            echo "<script> window.location = 'block_leader_home.php';</script>";
        ...
        echo "<script type='text/javascript'> $(document).ready(function(){ 
                swal({
                         title: 'Sorry!',
                         text: 'SEAFB (RDCSAC) App is closed for now',
                         icon: 'error',
                        buttons: {
...
<head>
    <title>Block Leader Login</title>
                ...
                <div class="col-sm-12">
                    ...
                    <form class="md-float-material form-material" action="" method="POST">
                        <div class="text-center">
                            <i>_ <a href="index"><b style="font-weight: bolder; font-size: 24px; color: #ccffcc;">SEAFB
                                        (RDCSAC)</b></a>_</i>
                        </div>
                        <div class="auth-box card">
                            <div class="card-block">
                                <div class="row m-b-20">
                                    <div class="col-md-12">
                                        <h3 class="text-center">Block Leader Sign In</h3>
                                    </div>
                                </div>
